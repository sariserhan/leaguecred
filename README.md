...
Fixture synchronization runs three independent sources, each caught and reported separately so one being down for the night never stops the others: ESPN's public scoreboard API covers every enabled league and goes first; Football-Data.co.uk's published CSV files add whatever ESPN is missing across 12 domestic feeds; football-data.org adds the same for its free-tier competitions (the top-five European leagues plus the Championship, Primeira Liga, Eredivisie, Brasileirão, and Champions League) when `FOOTBALL_DATA_API_KEY` is configured. Whoever records a match first keeps it — the later sources only fill gaps, so a real gameweek never gets recorded twice. The files are parsed in memory and discarded; Neon stores normalized fixtures, scores, provider aliases, and sync-run metadata. API-Football remains available for catalog metadata but is no longer a fixture source.

Vercel runs two cron jobs: `GET /api/jobs/daily` at 04:00 UTC, which chains fixture synchronization, settlement, and team-logo backfill in that order, and `GET /api/jobs/reminders` at 09:00 UTC, which runs the lock-reminder and specialist-lock-notification steps. A Hobby project only gets two cron jobs, so a new kind of scheduled mail is a step added to one of these routes rather than a third cron. The individual routes `/api/jobs/fixtures`, `/api/jobs/settlement`, `/api/jobs/team-logos`, and `/api/jobs/reminders` remain available for manual runs. All of them accept GET or POST and require `Authorization: Bearer $CRON_SECRET`.
