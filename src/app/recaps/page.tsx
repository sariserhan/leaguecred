...
export default async function RecapsPage() {
  ...
  return <main>
    <header className="border-b bg-foreground text-background"><div className="page-shell py-10 sm:py-14"><h1 className="font-heading text-[clamp(3.8rem,9vw,8rem)] leading-[.84] font-extrabold uppercase">This week, honestly.</h1><div className="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-sm text-background/70"><span>{data.from} → {data.to}</span><span>{data.totals.specialists} participating specialists</span><span>Wins and misses included</span></div></div></header>
    <div className="page-shell py-8 sm:py-12">
      ...
      <section className="mt-7 grid gap-5 border bg-foreground p-6 text-background sm:grid-cols-[1fr_auto] sm:items-center"><div><h2 className="font-heading text-4xl font-bold uppercase">Build next week&apos;s record.</h2><p className="mt-2 text-background/70">One selective call from the league you genuinely follow. Completely free.</p></div><Link href="/leagues?intent=prove" className={buttonVariants({size:"lg"})}>Make a Daily Lock</Link></section>
    </div>
  </main>;
}
