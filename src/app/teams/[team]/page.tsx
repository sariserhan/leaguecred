...
function TeamMark({ name, logoUrl }: { name: string; logoUrl: string | null }) {
  return logoUrl ? <Image src={logoUrl} alt="" width={40} height={40} className="size-10 object-contain" /> : <span className="flex size-10 items-center justify-center bg-muted text-xs font-bold">{name.slice(0, 3).toUpperCase()}</span>;
}

function FixtureRow({ fixture, recent }: { fixture: TeamFixture; recent?: boolean }) {
  ...
  const result = fixture.teamScore === null || fixture.opponentScore === null ? "—" : `${fixture.teamScore}–${fixture.opponentScore}`;
