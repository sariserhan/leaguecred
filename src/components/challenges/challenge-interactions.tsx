...
export function ChallengeShareCard({ challengeId, homeName, awayName, representedName, representedSlug }: { challengeId: string; homeName: string; awayName: string; representedName: string | null; representedSlug: string | null }) {
  ...
  return <section className="mt-6 grid border bg-foreground text-background lg:grid-cols-[1fr_auto]" aria-labelledby="share-challenge-heading">
    <div className="p-6 sm:p-8"><p className="text-xs font-bold tracking-[.14em] text-primary uppercase">Shareable challenge card</p><h2 id="share-challenge-heading" className="mt-3 font-heading text-4xl font-extrabold uppercase sm:text-5xl">{representedName ? `${representedName} stands here.` : `${homeName} vs ${awayName}`}</h2><p className="mt-3 max-w-2xl text-background/70">{representedName ? `Invite another ${representedName} supporter and build your side with people who know the club.` : "Bring the supporters who know these clubs best into the challenge."}</p></div>
    <div className="flex flex-col justify-center gap-3 border-t border-background/20 p-6 lg:min-w-64 lg:border-t-0 lg:border-l"><Button onClick={share}><Share2Icon data-icon="inline-start" />Share challenge</Button><Button variant="outline" onClick={copyInvite} className="border-background/30 bg-transparent text-background hover:bg-background hover:text-foreground">{copied ? <CheckIcon data-icon="inline-start" /> : <CopyIcon data-icon="inline-start" />}{copied ? "Invite copied" : "Copy invitation"}</Button></div>
  </section>;
}
