    ...
    <header className="border-b bg-background">
      <div className="page-shell flex h-16 items-center justify-between gap-3 sm:h-20">
          ...
          <Dialog>
            ...
            <DialogContent className="inset-y-0 left-0 h-dvh max-w-[min(88vw,380px)] translate-x-0 translate-y-0 content-start rounded-none p-0 sm:max-w-[380px]" showCloseButton>
              ...
              <nav className="divide-y" aria-label="Mobile navigation">
                ...
                {liveLocksEnabled ? <DialogClose nativeButton={false} render={<Link href="/live-locks" className="flex min-h-16 items-center gap-3 px-5 font-semibold hover:bg-muted" />}><RadioIcon className="size-5 text-primary" />Global active locks<ChevronRightIcon className="ml-auto size-5 text-muted-foreground" /></DialogClose> : null}
        ...
        <div className="flex items-center gap-1.5"><GlobalSearch /><ThemeToggle className="hidden sm:inline-flex" />{session && notificationCenter ? <NotificationCenter initialItems={notificationCenter.items} initialPreferences={notificationCenter.preferences} /> : null}{session ? (
          <DropdownMenu>
            <DropdownMenuTrigger render={<Button variant="ghost" size="icon" className="cursor-pointer rounded-full" aria-label={`Open account menu for ${session.user.name}`} />}><Avatar className="size-8"><AvatarFallback>{initials}</AvatarFallback></Avatar></DropdownMenuTrigger>
