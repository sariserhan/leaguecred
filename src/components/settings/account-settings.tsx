...
import { BellRingIcon, ClockIcon, KeyRoundIcon, PaletteIcon, ShieldAlertIcon, UserRoundIcon, UsersRoundIcon } from "lucide-react";
...
export function AccountSettings({ user, initialPreferences }: { user: { id: string; name: string; email: string }; initialPreferences: NotificationPreferences }) {
  ...
  const [profileTheme, setProfileTheme] = useState("Pitch dark");
  ...
  return <main className="page-shell py-8 sm:py-12">
    ...
    <div className="mt-4 grid gap-5 lg:grid-cols-2">
      ...
      <Card className="rounded-none"><CardHeader><CardTitle className="font-heading text-3xl font-bold uppercase">Profile presentation</CardTitle><CardDescription>Preview how your public record introduces you.</CardDescription><CardAction><PaletteIcon className="size-6 text-primary"/></CardAction></CardHeader><CardContent className="grid gap-4"><Field><FieldLabel htmlFor="bio">Short bio</FieldLabel><Input id="bio" value={bio} onChange={e=>setBio(e.target.value)} maxLength={120} placeholder="The league and perspective you know best"/><FieldDescription>{bio.length}/120 characters</FieldDescription></Field><Field><FieldLabel htmlFor="profile-theme">Record-card theme</FieldLabel><select id="profile-theme" value={profileTheme} onChange={e=>setProfileTheme(e.target.value)} className="h-10 border bg-background px-3"><option>Pitch dark</option><option>Paper light</option><option>High contrast</option></select></Field><div className={profileTheme==="Paper light"?"border p-5":"border bg-foreground p-5 text-background"}><span className="text-xs font-bold uppercase text-primary">Public profile preview</span><strong className="mt-2 block font-heading text-3xl uppercase">{user.name}</strong><p className="mt-2 text-sm opacity-70">{bio||"Add a short bio to introduce your football perspective."}</p></div><p className="text-xs text-muted-foreground">Presentation choices are preview-only until public profile metadata is enabled.</p></CardContent></Card>
