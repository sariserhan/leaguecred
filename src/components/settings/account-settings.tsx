...
import { BellRingIcon, ClockIcon, KeyRoundIcon, LaptopIcon, MoonIcon, ShieldAlertIcon, SunIcon, UserRoundIcon, UsersRoundIcon } from "lucide-react";
...
export function AccountSettings({ user, leagues, initialPreferences }: { user: { id:string;name:string;username:string|null;email:string;bio:string|null;image:string|null;profile_theme:string;featured_league_id:string|null;pinned_milestone:string|null }; leagues:Array<{id:string;name:string}>; initialPreferences: NotificationPreferences }) {
  ...
  function saveProfile(event: FormEvent<HTMLFormElement>) {
    ...
    const data=new FormData(event.currentTarget);const name=String(data.get("name")??"");const handle=String(data.get("handle")??"");
    startTransition(async () => {
      const result = await updateProfile({ name,handle,bio:String(data.get("bio")??""),image:String(data.get("image")??""),profileTheme:data.get("profileTheme") as "pitch-dark"|"paper-light"|"high-contrast",featuredLeagueId:String(data.get("featuredLeagueId")||"")||null,pinnedMilestone:String(data.get("pinnedMilestone")||"")||null });
  ...
  return <main className="page-shell py-8 sm:py-12">
    ...
    <div className="mt-4 grid gap-5 lg:grid-cols-2">
      ...
      <Card className="rounded-none lg:col-span-2"><CardHeader><CardTitle className="font-heading text-3xl font-bold uppercase">Appearance</CardTitle><CardDescription>Choose your interface theme across all devices.</CardDescription><CardAction><SunIcon className="size-6 text-primary"/></CardAction></CardHeader><CardContent className="grid gap-4"><div className="grid grid-cols-3 gap-3"><Button type="button" variant={theme === "light" ? "default" : "outline"} onClick={() => setTheme("light")} className="flex flex-col items-center gap-1.5 h-auto py-3.5"><SunIcon className="size-5" /><span className="text-xs font-bold uppercase">Light</span></Button><Button type="button" variant={theme === "dark" ? "default" : "outline"} onClick={() => setTheme("dark")} className="flex flex-col items-center gap-1.5 h-auto py-3.5"><MoonIcon className="size-5" /><span className="text-xs font-bold uppercase">Dark</span></Button><Button type="button" variant={theme === "system" ? "default" : "outline"} onClick={() => setTheme("system")} className="flex flex-col items-center gap-1.5 h-auto py-3.5"><LaptopIcon className="size-5" /><span className="text-xs font-bold uppercase">System</span></Button></div><p className="text-xs text-muted-foreground">Currently active: <strong className="capitalize text-foreground">{resolvedTheme}</strong> mode {theme === "system" ? "(syncing with your device system settings)" : ""}</p></CardContent></Card>
