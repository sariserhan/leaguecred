...
export function OnboardingFlow({ leagues, teams, initialPreferences, initialTeamId, initialRegion }: { leagues: League[]; teams: IdentityTeam[]; initialPreferences: LeaguePreferences; initialTeamId: string | null; initialRegion: string | null }) {
  ...
  return <main className="page-shell max-w-4xl py-12 sm:py-20">
    ...
    </section> : <section className="mt-10 grid gap-6 border bg-inverted p-7 text-inverted-foreground md:grid-cols-[1fr_auto] md:items-center"><div><h2 className="section-title">You represent {chosenTeam?.name ?? "your community"}.</h2><p className="mt-3 text-inverted-foreground/70">Your football identity and specialist network are ready. Your first contribution is one selective Daily Lock.</p></div><div className="flex flex-col gap-3"><Link href={`/leagues/${known[0]}`} className="inline-flex h-12 items-center justify-center bg-primary px-6 font-bold text-primary-foreground">Make my first Daily Lock</Link><Link href="/invite" className="text-center text-sm font-semibold text-primary underline">Invite another supporter</Link></div></section>}
  </main>;
}
