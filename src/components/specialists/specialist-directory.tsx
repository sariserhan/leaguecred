...
export function SpecialistDirectory({ specialists, initialLeague, rankThreshold }: { specialists: SpecialistDirectoryEntry[]; initialLeague?: string; rankThreshold: number }) {
  ...
  return (
    <div className={cn("page-shell py-14 sm:py-20", compared.length > 0 && "pb-36")}>
      ...
      {specialists.some((entry) => entry.provisional) ? <div className="mt-8 border border-primary p-5"><div className="flex flex-wrap items-center gap-2"><Badge>Founding season</Badge><strong>Evidence is still forming</strong></div><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Provisional records remain visible so early contributors have a path forward, but they cannot be followed or publicly ranked until they reach {rankThreshold} settled independent Daily Locks.</p></div> : null}
      {filtered.length ? <section className="mt-9" aria-labelledby="ranking-heading"><div className="flex items-end justify-between gap-4"><h2 id="ranking-heading" className="font-heading text-3xl font-bold uppercase">Specialist records</h2><span className="text-sm text-muted-foreground">{filtered.length} result{filtered.length === 1 ? "" : "s"}</span></div><div className="mt-3 border-y lg:grid lg:grid-cols-[48px_1.3fr_1fr_.7fr_.8fr_.8fr_.7fr_.7fr_auto_auto] lg:gap-x-4"><div className="hidden border-b bg-muted px-4 py-3 text-xs font-bold uppercase text-muted-foreground lg:col-span-full lg:grid lg:grid-cols-[subgrid] lg:gap-x-4"><span>#</span><span>Specialist</span><span>Strongest league</span><span>Accuracy</span><span>Record</span><span>Evidence</span><span>Streak</span><span>Followers</span><span>View</span><span>Compare</span></div><ol className="lg:contents">{filtered.map((entry, index) => <SpecialistRow key={entry.id} entry={entry} rank={index + 1} selected={comparedIds.includes(entry.id)} compareFull={comparedIds.length >= 3} onCompare={() => toggleCompare(entry.id)} rankThreshold={rankThreshold} />)}</ol></div></section> : <div className="mt-9 flex min-h-72 flex-col items-center justify-center border-y text-center"><SearchIcon className="size-10" /><h2 className="mt-5 font-heading text-3xl font-bold uppercase">No records found</h2><p className="mt-2 max-w-md text-sm text-muted-foreground">No independent Daily Locks match these filters yet. Clear filters or become this league&apos;s first contributor.</p><Button variant="outline" onClick={reset} className="mt-6"><RotateCcwIcon data-icon="inline-start" />Clear filters</Button></div>}
      ...
      {compared.length ? <aside className="fixed inset-x-0 bottom-0 z-40 border-t border-primary bg-inverted text-inverted-foreground"><div className="page-shell flex min-h-24 items-center gap-3 py-3"><div className="mr-auto"><strong className="font-heading text-2xl uppercase">Compare {compared.length}</strong><p className="hidden text-xs text-inverted-foreground/70 sm:block">Select up to three specialists.</p></div>{compared.map((entry) => <button key={entry.id} type="button" onClick={() => toggleCompare(entry.id)} className="hidden items-center gap-2 border border-inverted-foreground/25 px-3 py-2 text-sm sm:flex"><Avatar className="size-7"><AvatarFallback>{entry.initials}</AvatarFallback></Avatar>{entry.name}<XIcon className="size-4" /></button>)}<Button disabled={compared.length < 2} onClick={() => setCompareOpen(true)}><GitCompareArrowsIcon data-icon="inline-start" />Compare</Button></div></aside> : null}
    </div>
  );
}
...
function SpecialistRow({ entry, rank, selected, compareFull, onCompare, rankThreshold }: { entry: SpecialistDirectoryEntry; rank: number; selected: boolean; compareFull: boolean; onCompare: () => void; rankThreshold: number }) {
  ...
  return <li className="grid gap-4 border-b px-4 py-5 last:border-b-0 lg:col-span-full lg:grid-cols-[subgrid] lg:items-center lg:gap-x-4"><span className="font-heading text-2xl text-muted-foreground">{entry.provisional ? "—" : rank}</span><span className="flex items-center gap-3"><Avatar><AvatarFallback>{entry.initials}</AvatarFallback></Avatar><span><strong className="block">{entry.name}</strong>{entry.provisional ? <Badge variant="outline">Provisional · {rankThreshold - entry.settledPicks} to rank</Badge> : null}</span></span><Link href={`/leagues/${entry.leagueSlug}`} className="font-semibold hover:text-primary">{entry.leagueName}</Link><dl className="col-span-full grid grid-cols-2 gap-x-4 gap-y-3 border-y py-3 sm:grid-cols-5 lg:contents">{metrics.map(([label, value], index) => <div key={label}><dt className="text-[10px] font-bold uppercase text-muted-foreground lg:sr-only">{label}</dt><dd className={cn(index === 0 && "font-heading text-3xl font-bold text-primary", index === 4 && "flex items-center gap-2")}>{index === 4 ? <UsersRoundIcon className="size-4" /> : null}{value}</dd></div>)}</dl><Link href={`/specialists/${entry.handle ?? entry.id}`} className="inline-flex min-h-10 items-center justify-center gap-2 border border-inverted px-4 text-sm font-semibold hover:bg-inverted hover:text-inverted-foreground">View profile<ArrowRightIcon className="size-4" /></Link><Button variant={selected ? "default" : "outline"} size="sm" disabled={entry.provisional || (!selected && compareFull)} onClick={onCompare} aria-pressed={selected}>{entry.provisional ? "Building record" : selected ? <><CheckIcon data-icon="inline-start" />Selected</> : <><GitCompareArrowsIcon data-icon="inline-start" />Compare</>}</Button></li>;
}

function ComparisonCard({ entry }: { entry: SpecialistDirectoryEntry }) { const accuracy = entry.settledPicks ? (entry.wins / entry.settledPicks) * 100 : 0;const metrics=[['Accuracy',accuracy,100,`${accuracy.toFixed(1)}%`],['Adjusted',entry.confidenceAdjustedAccuracy*100,100,`${(entry.confidenceAdjustedAccuracy*100).toFixed(1)}%`],['Evidence',entry.settledPicks,50,`${entry.settledPicks} locks`],['Current form',entry.currentWinStreak,10,`${entry.currentWinStreak}W`]] as const;return <article className="border p-5"><div className="flex items-center gap-3"><Avatar><AvatarFallback>{entry.initials}</AvatarFallback></Avatar><div><strong>{entry.name}</strong><p className="text-sm text-muted-foreground">{entry.leagueName}</p></div></div><dl className="mt-5 grid gap-4">{metrics.map(([label,value,max,text])=><div key={label}><div className="flex justify-between text-xs"><dt className="text-muted-foreground">{label}</dt><dd className="font-semibold">{text}</dd></div><div className="mt-1 h-2 bg-muted"><span className="block h-full bg-primary" style={{width:`${Math.min(100,value/max*100)}%`}}/></div></div>)}<div className="flex justify-between border-t pt-3"><dt className="text-xs text-muted-foreground">Followers</dt><dd className="font-semibold">{entry.followers}</dd></div></dl><Link href={`/specialists/${entry.handle ?? entry.id}`} className="mt-5 flex items-center gap-2 font-semibold hover:text-primary">Open profile<ArrowRightIcon className="size-4" /></Link></article>; }
