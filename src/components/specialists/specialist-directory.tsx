...
export function SpecialistDirectory({ specialists, initialLeague }: { specialists: SpecialistDirectoryEntry[]; initialLeague?: string }) {
  ...
  return (
    <main className={cn("page-shell py-14 sm:py-20", compared.length > 0 && "pb-36")}>
      ...
      {specialists.some((entry) => entry.provisional) ? <div className="mt-8 border border-primary p-5"><div className="flex flex-wrap items-center gap-2"><Badge>Founding season</Badge><strong>Evidence is still forming</strong></div><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Provisional records remain visible so early contributors have a path forward, but they cannot be followed or publicly ranked until they reach {MINIMUM_SETTLED_PICKS_FOR_RANK} settled independent Weekly Locks.</p></div> : null}
      {filtered.length ? <section className="mt-9" aria-labelledby="ranking-heading"><div className="flex items-end justify-between gap-4"><h2 id="ranking-heading" className="font-heading text-3xl font-bold uppercase">Specialist records</h2><span className="text-sm text-muted-foreground">{filtered.length} result{filtered.length === 1 ? "" : "s"}</span></div><div className="mt-3 border-y"><div className="hidden grid-cols-[48px_1.3fr_1fr_.7fr_.8fr_.8fr_.7fr_.7fr_auto_auto] gap-4 border-b bg-muted px-4 py-3 text-xs font-bold uppercase text-muted-foreground lg:grid"><span>#</span><span>Specialist</span><span>Strongest league</span><span>Accuracy</span><span>Record</span><span>Evidence</span><span>Streak</span><span>Followers</span><span>View</span><span>Compare</span></div><ol>{filtered.map((entry, index) => <SpecialistRow key={entry.id} entry={entry} rank={index + 1} selected={comparedIds.includes(entry.id)} compareFull={comparedIds.length >= 3} onCompare={() => toggleCompare(entry.id)} />)}</ol></div></section> : <div className="mt-9 flex min-h-72 flex-col items-center justify-center border-y text-center"><SearchIcon className="size-10" /><h2 className="mt-5 font-heading text-3xl font-bold uppercase">No records found</h2><p className="mt-2 max-w-md text-sm text-muted-foreground">No independent Weekly Locks match these filters yet. Clear filters or become this league&apos;s first contributor.</p><Button variant="outline" onClick={reset} className="mt-6"><RotateCcwIcon data-icon="inline-start" />Clear filters</Button></div>}
      ...
      <Dialog open={compareOpen} onOpenChange={setCompareOpen}><DialogContent className="max-w-3xl rounded-none"><DialogHeader><DialogTitle className="font-heading text-3xl font-bold uppercase">Specialist comparison</DialogTitle><DialogDescription>Independent evidence, shown side by side.</DialogDescription></DialogHeader><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{compared.map((entry) => <ComparisonCard key={entry.id} entry={entry} />)}</div></DialogContent></Dialog>
...
function SpecialistRow({ entry, rank, selected, compareFull, onCompare }: { entry: SpecialistDirectoryEntry; rank: number; selected: boolean; compareFull: boolean; onCompare: () => void }) {
  ...
  return <li className="grid gap-4 border-b px-4 py-5 last:border-b-0 lg:grid-cols-[48px_1.3fr_1fr_.7fr_.8fr_.8fr_.7fr_.7fr_auto_auto] lg:items-center lg:gap-4"><span className="font-heading text-2xl text-muted-foreground">{entry.provisional ? "—" : rank}</span><span className="flex items-center gap-3"><Avatar><AvatarFallback>{entry.initials}</AvatarFallback></Avatar><span><strong className="block">{entry.name}</strong>{entry.provisional ? <Badge variant="outline">Provisional · {MINIMUM_SETTLED_PICKS_FOR_RANK - entry.settledPicks} to rank</Badge> : null}</span></span><Link href={`/leagues/${entry.leagueSlug}`} className="font-semibold hover:text-primary">{entry.leagueName}</Link><dl className="col-span-full grid grid-cols-2 gap-x-4 gap-y-3 border-y py-3 sm:grid-cols-5 lg:contents">{metrics.map(([label, value], index) => <div key={label}><dt className="text-[10px] font-bold uppercase text-muted-foreground lg:sr-only">{label}</dt><dd className={cn(index === 0 && "font-heading text-3xl font-bold text-primary", index === 4 && "flex items-center gap-2")}>{index === 4 ? <UsersRoundIcon className="size-4" /> : null}{value}</dd></div>)}</dl><Link href={`/specialists/${entry.id}`} className="inline-flex min-h-10 items-center justify-center gap-2 border border-foreground px-4 text-sm font-semibold hover:bg-foreground hover:text-background">View profile<ArrowRightIcon className="size-4" /></Link><Button variant={selected ? "default" : "outline"} size="sm" disabled={entry.provisional || (!selected && compareFull)} onClick={onCompare} aria-pressed={selected}>{entry.provisional ? "Building record" : selected ? <><CheckIcon data-icon="inline-start" />Selected</> : <><GitCompareArrowsIcon data-icon="inline-start" />Compare</>}</Button></li>;
}

function ComparisonCard({ entry }: { entry: SpecialistDirectoryEntry }) { const accuracy = entry.settledPicks ? (entry.wins / entry.settledPicks) * 100 : 0; return <article className="border p-5"><div className="flex items-center gap-3"><Avatar><AvatarFallback>{entry.initials}</AvatarFallback></Avatar><div><strong>{entry.name}</strong><p className="text-sm text-muted-foreground">{entry.leagueName}</p></div></div><dl className="mt-5 grid grid-cols-2 gap-4"><div><dt className="text-xs text-muted-foreground">Accuracy</dt><dd className="font-heading text-3xl text-primary">{accuracy.toFixed(1)}%</dd></div><div><dt className="text-xs text-muted-foreground">Adjusted</dt><dd className="font-heading text-3xl">{(entry.confidenceAdjustedAccuracy * 100).toFixed(1)}%</dd></div><div><dt className="text-xs text-muted-foreground">Evidence</dt><dd className="font-semibold">{entry.settledPicks} locks</dd></div><div><dt className="text-xs text-muted-foreground">Followers</dt><dd className="font-semibold">{entry.followers}</dd></div></dl><Link href={`/specialists/${entry.id}`} className="mt-5 flex items-center gap-2 font-semibold hover:text-primary">Open profile<ArrowRightIcon className="size-4" /></Link></article>; }
