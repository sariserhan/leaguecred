...
export function DashboardActivity({ data }: { data: SpecialistProfileData }) {
  const activity = [
    ...
    ...data.followedHistory.map((entry) => ({ id: `followed-${entry.id}`, at: entry.followedAt, icon: UserRoundCheckIcon, title: `Followed ${entry.specialistName}'s ${entry.team} call`, body: `${entry.leagueName} · ${entry.result}`, href: `/specialists/${entry.specialistHandle ?? entry.specialistId}` })),
