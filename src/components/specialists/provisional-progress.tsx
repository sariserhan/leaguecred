...
export function ProvisionalProgress({ leagues }: { leagues: SpecialistProfileData["leagues"] }) {
  ...
  return <section className="mb-8 border border-foreground" aria-labelledby="specialist-path-heading">
    <header className="grid gap-4 border-b bg-muted p-5 sm:grid-cols-[1fr_auto] sm:items-end sm:p-7">
      <div><p className="flex items-center gap-2 text-xs font-bold tracking-[0.12em] text-primary uppercase"><FlagIcon aria-hidden="true" className="size-4" />First-season progress</p><h2 id="specialist-path-heading" className="mt-2 font-heading text-4xl leading-none font-extrabold uppercase sm:text-5xl">Build evidence. Earn your rank.</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">Each league needs {MINIMUM_SETTLED_PICKS_FOR_RANK} settled independent Weekly Locks before your record can enter specialist rankings. Followed calls never count toward this progress.</p></div>
    ...
    })}</div> : <div className="p-6 sm:p-8"><strong className="font-heading text-3xl uppercase">Your first result starts the record.</strong><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Choose a league you know, submit one near-certain Weekly Lock, and this dashboard will track every settled step toward ranking eligibility.</p></div>}
