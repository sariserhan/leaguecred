...
export type GlobalActiveLock = {
  ...
  opponent: { name: string; slug: string; logoUrl: string | null };
...
export async function getGlobalActiveLocks(viewerId?: string): Promise<GlobalActiveLock[]> {
  const locks = await sqlClient<Array<Omit<GlobalActiveLock, "league" | "selected" | "opponent" | "home" | "away" | "opinions"> & {
    league_name: string; league_slug: string; selected_name: string; selected_slug: string; selected_logo: string | null;
    opponent_name: string; opponent_slug: string; opponent_logo: string | null; home_name: string; home_logo: string | null; away_name: string; away_logo: string | null;
  }>>`
    select p.id, p.user_id "userId", u.name username, u.image "userImage", p.decision_reason message,
      p.locked_at::text "lockedAt", f.kickoff_at::text "kickoffAt", l.name league_name, l.slug league_slug,
      st.name selected_name, st.slug selected_slug, st.logo_url selected_logo,
  ...
  return locks.map((lock) => ({
    ...
    league: { name: lock.league_name, slug: lock.league_slug },
    selected: { name: lock.selected_name, slug: lock.selected_slug, logoUrl: lock.selected_logo },
    opponent: { name: lock.opponent_name, slug: lock.opponent_slug, logoUrl: lock.opponent_logo },
