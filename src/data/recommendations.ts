...
export const getPersonalizedRecommendations=cache(async(userId:string)=>{const rankThreshold=await getRankThreshold();return sqlClient<Array<{league_id:string;league_name:string;league_slug:string;specialist_id:string;specialist_name:string;wins:number;losses:number;settled_picks:number;current_win_streak:number;adjusted:string;followers:number;followed:boolean;rank:number}>>`
  with ranked as (
