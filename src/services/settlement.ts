...
type SettleablePick = {
  id: string; user_id: string; league_id: string; season_id: string; selected_team_id: string;
  fixture_status: string; winner_team_id: string | null;
};
...
export async function settlePendingPicks() {
  ...
  let settled = 0;
  for (const candidate of candidates) settled += await settlePick(candidate.id) ? 1 : 0;
...
export async function settlePick(pickId: string) {
  return sqlClient.begin(async (sql) => {
    const [pick] = await sql<SettleablePick[]>`
      ...
      from picks p join fixtures f on f.id = p.fixture_id where p.id = ${pickId} for update of p, f`;
    if (!pick || pick.fixture_status === "scheduled" || pick.fixture_status === "live" || pick.fixture_status === "postponed") return false;
    const [active] = await sql<Array<{ pick_id: string }>>`select pick_id from active_settlement_effects where pick_id = ${pick.id} for update`;
    ...
    const [event] = await sql<Array<{ id: string }>>`
      ...
      values (${pick.user_id}, ${pick.league_id}, ${pick.season_id}, ${pick.id}, 'initial_settlement', ${result}) returning id`;
...
export async function correctSettlement(pickId: string, reason: string) {
  ...
  return sqlClient.begin(async (sql) => {
    ...
    const [active] = await sql<Array<{ event_id: string; result: Exclude<PickResult, "pending"> }>>`
      select event_id, result from active_settlement_effects where pick_id = ${pick.id} for update`;
    ...
    const [reversal] = await sql<Array<{ id: string }>>`
      ...
      values (${pick.user_id}, ${pick.league_id}, ${pick.season_id}, ${pick.id}, 'reversal', ${active.result}, ${active.event_id}, ${reason.trim()}) returning id`;
    const [correction] = await sql<Array<{ id: string }>>`
      insert into settlement_events (user_id, league_id, season_id, pick_id, event_type, result, supersedes_event_id, reason)
      values (${pick.user_id}, ${pick.league_id}, ${pick.season_id}, ${pick.id}, 'correction', ${correctedResult}, ${reversal!.id}, ${reason.trim()}) returning id`;
    await sql`update active_settlement_effects set event_id = ${correction!.id}, result = ${correctedResult}, updated_at = now() where pick_id = ${pick.id}`;
    await applyResult(sql, pick, correctedResult);
    return true;
  });
}

async function applyResult(sql: postgres.TransactionSql, pick: SettleablePick, result: Exclude<PickResult, "pending">) {
  await sql`update picks set result = ${result}, settled_at = now(), updated_at = now() where id = ${pick.id}`;
  await sql`update followed_picks set result = ${result}, settled_at = now() where source_pick_id = ${pick.id}`;
...
async function rebuildRecord(sql: postgres.TransactionSql, userId: string, leagueId: string, seasonId: string | null) {
  ...
  const wins = events.filter((event) => event.result === "win").length;
  const losses = events.filter((event) => event.result === "loss").length;
  const voids = events.filter((event) => event.result === "void").length;
  let currentStreak = 0; let bestStreak = 0;
  for (const event of events) {
    if (event.result === "win") { currentStreak += 1; bestStreak = Math.max(bestStreak, currentStreak); }
    else if (event.result === "loss") currentStreak = 0;
  }
  const settledPicks = wins + losses;
  const score = wilsonLowerBound(wins, losses).toFixed(6);
  const tier = settledPicks < 10 ? "Provisional" : "Established";
  ...
  if (seasonId) {
    await sql`insert into user_league_season_records (user_id, league_id, season_id, wins, losses, voids, settled_picks, current_win_streak, best_win_streak, tier, confidence_adjusted_accuracy, last_settled_at)
      values (${userId}, ${leagueId}, ${seasonId}, ${wins}, ${losses}, ${voids}, ${settledPicks}, ${currentStreak}, ${bestStreak}, ${tier}, ${score}, ${new Date(lastSettledAt).toISOString()})
    ...
    await sql`insert into user_league_records (user_id, league_id, wins, losses, voids, settled_picks, current_win_streak, best_win_streak, tier, confidence_adjusted_accuracy, last_settled_at)
      values (${userId}, ${leagueId}, ${wins}, ${losses}, ${voids}, ${settledPicks}, ${currentStreak}, ${bestStreak}, ${tier}, ${score}, ${new Date(lastSettledAt).toISOString()})
