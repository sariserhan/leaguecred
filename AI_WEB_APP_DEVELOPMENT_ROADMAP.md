...
> A project-specific product specification is authoritative for product scope. This roadmap is a reference and quality checklist, not a requirement to build every listed capability. Omit sections and modules that do not apply, and never let this document silently expand an MVP.
...
Every web application should include the smallest secure internal admin surface required to operate its actual features, unless the project explicitly documents why one is unnecessary. Only applicable modules are required; do not scaffold payments, revenue, affiliates, email, mobile, AI, or marketing administration when those capabilities are outside the project specification.
...
If the project does not have an affiliate program, omit this module until the product specification or active roadmap introduces one.
